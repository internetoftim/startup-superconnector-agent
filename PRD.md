...
This is *not* outbound automation. The core claim is **brokered representation**:
...
investor's agent *could* talk directly; they don't, because neither trusts a
...
*safe*. That is the platform's defensibility, and it only exists once founders
...
time went to *"replying to the same people every day"* instead of *"getting new
people."* Corroborated by the founder's own experience organising 30+ AI events
and operating as an angel/scout/community builder.

**The structural insight:** a connector's real work is not sending messages —
it is *representing people's interests to each other* under constraints. That
...
**US-1** — *As an SSC, I want a single point-of-contact agent that manages my
social network so that I don't waste time switching.*
...
**US-2** — *As an SSC, I want the agent to represent me and be the intermediary
to other agents.*
...
**US-3** — *As an SSC, I want the agent to be able to easily create an event
...
**drafted** emails, seat reservations — as the *hands* of an agreement, never
as unsolicited outbound.

> **CPO note on US-3:** "email blasting" is deliberately downgraded to
> *drafted, consent-based communications*. Positioning Superconnector as a
...
**US-4 (Founder)** — *As a founder, I want my own agent to get me warm,
high-fit investor intros through a trusted matchmaker, without exposing my deck
or metrics to people I haven't approved.*
...
**US-5 (Investor)** — *As an investor, I want curated, thesis-fit dealflow
through a matchmaker I trust, without inbound spam, and without exposing my
private availability or committing capital via an agent.*
...
**US-6 (Matchmaker/SSC)** — *As the matchmaker, I want my agent to broker
between founders' and investors' agents — curating fit, enforcing both sides'
consent, and vouching only within bounds.*
...
1. Prove the representation primitive: two proxies, two mandates, one signed
   agreement.
2. Make autonomy limits and consent *visible product behaviour*, not policy
   text.
...
- **GBrain as the profile memory layer:** the representation profile persists
  ...
  GBrain is the natural home — the profile *is* an organisational-brain
...
- **vs AI SDR / outbound tools:** they act *on* humans, push one agenda. We
...
| **Prompt injection via counterparty messages** | Inbound agent messages are *data, never instructions*; guardrail checks run outside the model; mandate is immutable within a session |
...
- Demo runbook: `runbook/RUNBOOK-DELTA.md` · Video script: `pitch/video-script.md`
- Seed schemas: `seed/my_profile.json`, `seed/counterparty_profile.json`
